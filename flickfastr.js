...
jQuery.fn.flickfastr = function(identifier, api_key, options) {
  ...
  PHOTO_HTML = '<a id="photo/{id}" class="photo" target="_blank" href="{href}"><img title="{title}" src="{src}"></img><div class="title">{title}</div></a>'

  $el = this, $window = $(window), $document = $(document)
  ...
  flickr_get = function(method, params, callback) {
    url = FLICKR_API_URL + "&" + $.param($.extend({ api_key: api_key, method: method, per_page: 10 }, params))
